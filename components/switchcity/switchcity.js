...
Page({
  ...
  confirmLoc: function(e){
    eventChannel.emit('someEvent', {city: this.data.city, county: this.data.county})
    wx.navigateBack({
      complete: (res) => {},
    })
  },
  ...
  onLoad: function (options) {
    ...
    this.setData({
      ...
      cityList: cityList,
      naviHeight: appInstance.globalData.menu.bottom
    });
    this.getLocation();
  },
  ...
  selectCounty: function () {
    ...
    const that = this;
    wx.request({
      url: `https://apis.map.qq.com/ws/district/v1/getchildren?&id=${code}&key=W7IBZ-AV333-5BL34-3GSCE-CKT5F-5TFNQ`,
      success: function (res) {
        console.log('selectCounty', res)
        that.setData({
  ...
  reGetLocation: function () {
    ...
    console.log('reGetLocation', this.data.city, this.data.county)
    //返回首页
    wx.switchTab({
      url: '../index/index'
    })
